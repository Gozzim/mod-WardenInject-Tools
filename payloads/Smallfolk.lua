...
function dump_type:table(nmemo, memo, acc)
    ...
    acc[#acc] = acc[#acc] == ob and ob .. cb or cb
    return nmemo
end
...
local function expect_number(string, start)
    ...
    if nonzero_digit[head] then
        repeat i = i + 1
            head = string:sub(i, i) until not is_digit[head]
    elseif head == '0' then
        i = i + 1
        head = string:sub(i, i)
    else
...
local expect_object_head = { t = function(string, i)
    return true, i
end, f = function(string, i)
    return false, i
end, n = function(string, i)
    return nil, i
end, Q = function(string, i)
    return -(0 / 0), i
end, N = function(string, i)
    return 0 / 0, i
end, I = function(string, i)
    return 1 / 0, i
end, i = function(string, i)
    return -1 / 0, i
end, [sq] = function(string, i)
    local nexti = i - 1
    repeat nexti = string:find(sq, nexti + 1, true) + 1 until string:sub(nexti, nexti) ~= sq
    return string:sub(i, nexti - 2):gsub(sq .. sq, sq), nexti
end, ['0'] = function(string, i)
    return expect_number(string, i - 1)
end, [ob] = function(string, i, tables)
    local nt, k, v = {}
    local j = 1
    tables[#tables + 1] = nt
    if string:sub(i, i) == cb then
        return nt, i + 1
    end
    while true do
