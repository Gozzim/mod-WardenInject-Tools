...
std::string ExampleClient = R"(local AIO = AIO or require("AIO") local MyHandlers = AIO.AddHandlers("AIOExample", {}) FrameTest = CreateFrame("Frame", "FrameTest", UIParent, "UIPanelDialogTemplate") local frame = FrameTest frame:SetSize(200, 200) frame:RegisterForDrag("LeftButton") frame:SetPoint("CENTER") frame:SetToplevel(true) frame:SetClampedToScreen(true) frame:SetMovable(true) frame:EnableMouse(true) frame:SetScript("OnDragStart", frame.StartMoving) frame:SetScript("OnHide", frame.StopMovingOrSizing) frame:SetScript("OnDragStop", frame.StopMovingOrSizing) AIO.SavePosition(frame) function MyHandlers.ShowFrame(player) frame:Show() end local input = CreateFrame("EditBox", "InputTest", frame, "InputBoxTemplate") input:SetSize(100, 30) input:SetAutoFocus(false) input:SetPoint("CENTER", frame, "CENTER", 0, 50) input:SetScript("OnEnterPressed", input.ClearFocus) input:SetScript("OnEscapePressed", input.ClearFocus) local slider = CreateFrame("Slider", "SlideTest", frame, "OptionsSliderTemplate") slider:SetSize(100, 17) slider:SetPoint("CENTER", frame, "CENTER", 0, -50) slider:SetValueStep(0.5) slider:SetMinMaxValues(0, 100) slider:SetValue(50) slider.tooltipText = 'This is the Tooltip hint' _G[slider:GetName() .."Text"]:SetText(50) slider:SetScript("OnValueChanged", function(self) _G[slider:GetName() .."Text"]:SetText(self:GetValue()) end) _G[slider:GetName().."High"]:Hide() _G[slider:GetName().."Low"]:Hide() slider:Show() local button = CreateFrame("Button", "ButtonTest", frame) button:SetSize(100, 30) button:SetPoint("CENTER", frame, "CENTER") button:EnableMouse(true) button:SetScript("OnMouseUp", function() input:ClearFocus() end) local texture = button:CreateTexture("TextureTest") texture:SetAllPoints(button) texture:SetTexture(0.5, 1, 1, 0.5) button:SetNormalTexture(texture) local fontstring = button:CreateFontString("FontTest") fontstring:SetFont("Fonts\\FRIZQT__.TTF", 11, "OUTLINE, MONOCHROME") fontstring:SetShadowOffset(1, -1) button:SetFontString(fontstring) button:SetText("Test") local function OnClickButton(btn) AIO.Handle("AIOExample", "Print", btn:GetName(), input:GetText(), slider:GetValue()) end button:SetScript("OnClick", OnClickButton))";
...
static struct WardenLoader {
    ...
    std::map<std::string, WardenData> data = {
            {"lualzw", {1, false, true, lualzw, {}}},
            {"Smallfolk", {1, false, true, Smallfolk, {}}},
            {"Queue", {1, false, true, Queue, {}}},
            {"LibStub", {1, false, true, LibStub, {}}},
            {"LibWindow", {1, false, true, LibWindow, {}}},
            {"CMH", {1, false, true, CMH, {}}},
            {"StatPointUI", {1, false, true, StatPointUI, {}}},
            {"AIO", {1.74, false, true, AIO, {"AIO_sv", "AIO_sv_Addons"}}},
            {"TooltipMod", {1, false, true, TooltipMod, {}}},
            {"ExampleClient", {1, false, true, ExampleClient, {}}},
    };
} wardenLoader;
...
void WardenInjectMgr::SendLargePayload(Player* player, const std::string& addon, double version, bool cache, bool comp, const std::string& data)
{
