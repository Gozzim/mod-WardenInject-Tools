...
static struct WardenLoader {
    ...
    std::map<std::string, WardenPayload> data = {
            {"lualzw", {1, false, true, lualzw, {}}},
            {"Smallfolk", {1, false, true, Smallfork, {}}},
            ...
            {"AIO", {1, false, true, AIO, {"AIO_sv", "AIO_sv_Addons"}}},
    };
} wardenLoader;
...
void WardenInjectMgr::SendLargePayload(Player* player, const std::string& addon, int version, bool cache, bool comp, const std::string& data)
{
    ...
    for (size_t i = 0; i < chunks.size(); i++)
    {
        ...
        LOG_INFO("module", "Sending large payload part: {}", payload);
...
void WardenInjectMgr::SendPayloadInform(Player* player)
{
    ...
    for (const auto& v : wardenLoader.order) {
        ...
        std::string message = Acore::StringFormatFmt("_G['{}'].f.i('{}', {}, {}, {})", cGTN, t.payload, t.version, t.cached, t.compressed);
...
void WardenInjectMgr::SendSpecificPayload(Player* player, std::string payloadName)
{
    // If the player has any payloads already cached, then they will be loaded immediately
    // Otherwise, full payloads will be requested
    const auto& t = wardenLoader.data[payloadName];
    // register all saved variables if there are any
    for (const auto& var : t.savedVars) {
        SendAddonMessage("ws", "_G['" + cGTN + "'].f.r('" + var + "')", CHAT_MSG_WHISPER, player);
    }
    ...
    LOG_INFO("module", "Sending payload inform: {}", message);
    SendAddonMessage("ws", message, CHAT_MSG_WHISPER, player);
    SendLargePayload(player, payloadName, t.version, t.cached, t.compressed, t.payload);
}
...
/*
void WardenInjectMgr::OnAddonMessageReceived(Player* player, int _type, const std::string& header, const std::string& data, Player* target)
{
    if (player->GetName() == target->GetName() && _type == 7)
    {
        if (header == "wc")
        {
            if (data == "reload")
            ...
            else if (data.substr(0, 3) == "req")
            {
                ...
                auto it = wardenData.find(addon);
                if (it != wardenData.end())
                {
                    WardenData& data = it->second;
                    player->SendLargePayload(addon, data.version, data.cached, data.compressed, data.payload);
                }
            }
        }
    }
